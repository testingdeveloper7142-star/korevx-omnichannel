...
export class InstagramAdapter implements ISocialChannelAdapter {
  ...
  validateWebhookSignature(signature: string, rawBody: Buffer | string, secretKey: string): boolean {
    ...
    try {
      ...
      this.logger.error(`Error al validar firma de webhook de Instagram: ${err.message}`);
      return false;
    }
  }
  ...
  normalizeIncomingPayload(channelAccountId: string, rawPayload: any): CanonicalNormalizedEvent[] {
    ...
    for (const entry of rawPayload.entry) {
      ...
      // 2. Comentarios en Publicaciones y Reels
      if (entry.changes && Array.isArray(entry.changes)) {
        for (const change of entry.changes) {
          if (change.field === 'comments') {
            const comment = change.value;
            const senderId = comment.from?.id || 'unknown';
            const senderUsername = comment.from?.username || `user_${senderId.substring(0, 6)}`;
            const commentId = comment.id || `ig_comment_${Date.now()}`;
            ...
            events.push({
              ...
              timestamp: new Date(comment.created_time ? Number(comment.created_time) * 1000 : Date.now()),
              rawPayload: change,
            });
          }
        }
  ...
  async sendMessage(payload: OutgoingMessagePayload): Promise<SendMessageResult> {
    try {
      if (payload.interactionType === InteractionType.POST_COMMENT) {
        // Responder a comentario en Instagram (Reel o Post)
        const targetId = payload.parentCommentId || payload.postId;
        const url = `${this.baseUrl}/${this.graphApiVersion}/${targetId}/replies`;
        ...
        // Enviar Instagram Direct Message
      ...
      this.logger.error(`Error enviando mensaje vía Instagram API: ${err.response?.data?.error?.message || err.message}`);
      return {
        success: false,
        externalMessageId: '',
        timestamp: new Date(),
